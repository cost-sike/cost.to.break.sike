...
{ // Doubling of a Montgomery point in projective coordinates (X:Z).
  ...
  // Output: projective Montgomery x-coordinates Q = 2*P = (X2:Z2).
    ...
    fp2sqr_mont(t0, t0);                            // t0 = (X1+Z1)^2   /* parallel 1 */
    fp2sqr_mont(t1, t1);                            // t1 = (X1-Z1)^2   /* parallel 1 */
    mp2_sub_p2(t0, t1, t2);                         // t2 = (X1+Z1)^2-(X1-Z1)^2 
    fp2mul_mont(C24, t1, t1);                       // t1 = C24*(X1-Z1)^2                   /* parallel 2 */
    fp2mul_mont(A24plus, t2, t3);                   // t3 = A24plus*[(X1+Z1)^2-(X1-Z1)^2]   /* parallel 2 */
    mp2_add(t1, t3, t3);                            // t3 = A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2
    fp2mul_mont(t0, t1, Q->X);                      // X2 = C24*(X1-Z1)^2*(X1+Z1)^2                                                 /* parallel 3 */
    fp2mul_mont(t2, t3, Q->Z);                      // Z2 = [A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2]*[(X1+Z1)^2-(X1-Z1)^2]   /* parallel 3 */
#endif
}
...
{ // Computes the corresponding 4-isogeny of a projective Montgomery point (X4:Z4) of order 4.
  ...
  //         that are used to evaluate the isogeny at a point in eval_4_isog().
    ...
    mp2_sub_p2(P->X, P->Z, coeff[1]);               // coeff[1] = X4-Z4
    mp2_add(P->X, P->Z, coeff[2]);                  // coeff[2] = X4+Z4
    fp2sqr_mont(P->Z, t0);                          // t0 = Z4^2    /* parallel 1 */
    fp2sqr_mont(P->X, t1);                          // t1 = X4^2    /* parallel 1 */
    mp2_add(t0, t0, t0);                            // t0 = 2*Z4^2
    mp2_add(t1, t1, t1);                            // t1 = 2*X4^2
    fp2sqr_mont(t1, A24plus);                       // A24plus = 4*X4^4   /* parallel 2 */
    fp2sqr_mont(t0, C24);                           // C24 = 4*Z4^4       /* parallel 2 */
    mp2_add(t0, t0, coeff[0]);                      // coeff[0] = 4*Z4^2
#endif
}
...
{ // Evaluates the isogeny at the point (X:Z) in the domain of the isogeny, given a 4-isogeny phi defined 
  ...
  // Output: the projective point P = phi(P) = (X:Z) in the codomain.
    ...
    mp2_add(P->X, P->Z, t0);                        // t0 = X+Z
    mp2_sub_p2(P->X, P->Z, t1);                     // t1 = X-Z
    fp2mul_mont(coeff[0], t0, t2);                  // t2 = coeff[0]*(X+Z)   /* parallel 1 */
    fp2mul_mont(coeff[1], t0, t3);                  // t3 = (X+Z)*coeff[1]   /* parallel 1 */
    fp2mul_mont(coeff[2], t1, t0);                  // t0 = (X-Z)*coeff[2]         /* parallel 2 */
    fp2mul_mont(t1, t2, t4);                        // t4 = coeff[0]*(X+Z)*(X-Z)   /* parallel 2 */
    mp2_add(t0, t3, t1);                            // t1 = (X-Z)*coeff[2] + (X+Z)*coeff[1]
    mp2_sub_p2(t0, t3, t0);                         // t0 = (X-Z)*coeff[2] - (X+Z)*coeff[1]
    fp2sqr_mont(t1, t1);                            // t1 = [(X-Z)*coeff[2] + (X+Z)*coeff[1]]^2   /* parallel 3 */
    fp2sqr_mont(t0, t0);                            // t0 = [(X-Z)*coeff[2] - (X+Z)*coeff[1]]^2   /* parallel 3 */
    mp2_add(t1, t4, t3);                            // t3 = coeff[0]*(X+Z)*(X-Z) + [(X-Z)*coeff[2] + (X+Z)*coeff[1]]^2
    mp2_sub_p2(t0, t4, t4);                         // t4 = [(X-Z)*coeff[2] - (X+Z)*coeff[1]]^2 - coeff[0]*(X+Z)*(X-Z)
    fp2mul_mont(t1, t3, P->X);                      // Xfinal   /* parallel 4 */
    fp2mul_mont(t0, t4, P->Z);                      // Zfinal   /* parallel 4 */
#endif
}
...
{ // Tripling of a Montgomery point in projective coordinates (X:Z).
  ...
  // Output: projective Montgomery x-coordinates Q = 3*P = (X3:Z3).    
    ...
    mp2_add(P->X, P->Z, t0);                        // t0 = X+Z 
    mp2_add(P->X, P->X, t1);                        // t1 = 2*X
    fp2sqr_mont(t0, t2);                            // t2 = (X+Z)^2   /* parallel 1 */
    fp2sqr_mont(t1, t3);                            // t3 = 4*X^2     /* parallel 1 */
    mp2_sub_p2(t3, t2, t0);                         // t0 = 4*X^2 - (X+Z)^2 
    mp2_sub_p2(P->X, P->Z, t4);                     // t4 = X-Z 
    fp2sqr_mont(t4, t3);                            // t3 = (X-Z)^2           /* parallel 2 */
    fp2mul_mont(A24plus, t2, t5);                   // t5 = A24plus*(X+Z)^2   /* parallel 2 */
    fp2mul_mont(t2, t5, t4);                        // t4 = A24plus*(X+Z)^4   /* parallel 3 */
    fp2mul_mont(A24minus, t3, t6);                  // t6 = A24minus*(X-Z)^2  /* parallel 3 */
    mp2_sub_p2(t0, t3, t0);                         // t0 = 4*X^2 - (X+Z)^2 - (X-Z)^2
    mp2_sub_p2(t5, t6, t5);                         // t5 = A24plus*(X+Z)^2 - A24minus*(X-Z)^2
    fp2mul_mont(t0, t5, t0);                        // t0 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]   /* parallel 4 */
    fp2mul_mont(t3, t6, t5);                        // t5 = A24minus*(X-Z)^4                                                   /* parallel 4 */
    mp2_sub_p2(t5, t4, t2);                         // t2 = A24minus*(X-Z)^4 - A24plus*(X+Z)^4
    mp2_add(P->Z, P->Z, t3);                        // t3 = 2*Z 
    fp2sub(t2, t0, t4);                             // t4 = A24minus*(X-Z)^4 - A24plus*(X+Z)^4 - [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]
    fp2add(t2, t0, t5);                             // t5 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2] + A24minus*(X-Z)^4 - A24plus*(X+Z)^4
    fp2sqr_mont(t4, t0);                            // t0 = t4^2      /* parallel 5 */
    fp2sqr_mont(t5, t2);                            // t2 = t5^2      /* parallel 5 */
    fp2mul_mont(t1, t2, Q->X);                      // X3 = 2*X*t2    /* parallel 6 */
    fp2mul_mont(t0, t3, Q->Z);                      // Z3 = 2*Z*t1    /* parallel 6 */
#endif
}
...
{ // Computes the corresponding 3-isogeny of a projective Montgomery point (X3:Z3) of order 3.
  ...
  // Output: the 3-isogenous Montgomery curve with projective coefficient A/C.   
    ...
    fp2add(P->X, P->X, t0);                         // t0 = 2*X        //////// NOTE: THIS FP2ADD AND FP2SUB CAN BE OPTIMIZED IF
    fp2sub(P->X, P->Z, coeff[0]);                   // coeff0 = X-Z    //////// SQR IS IMPLEMENTED AS A MULT.
    mp2_add(P->Z, P->Z, t1);                        // t1 = 2*Z
    mp2_add(t1, t1, t1);                            // t1 = 4*Z
    mp2_add(t0, coeff[0], t2);                      // t2 = 3*X-Z
    mp2_add(P->X, P->Z, coeff[1]);                  // coeff1 = X+Z
    fp2sqr_mont(t1, t1);                            // t1 = 16*Z^2          /* parallel 1 */
    fp2sqr_mont(t2, t0);                            // t0 = (3*X-Z)^2       /* parallel 1 */
    fp2mul_mont(t2, coeff[1], t2);                  // t2 = (3*X-Z)*(X+Z)   /* parallel 2 */
    fp2mul_mont(P->X, P->Z, t3);                    // t3 = XZ              /* parallel 2 */
    fp2mul_mont(t1, t3, t1);                        // t1 = 16*XZ^3                 /* parallel 3 */
    fp2mul_mont(t0, t2, A24minus);                  // A24minus = (3*X-Z)^3*(X+Z)   /* parallel 3 */
    mp2_sub_p2(A24minus, t1, A24plus);              // A24plus = (3*X-Z)^3*(X+Z)-16*XZ^3
#endif
}
...
{ // Given the x-coordinates of P, Q, and R, returns the value A corresponding to the Montgomery curve E_A: y^2=x^3+A*x^2+x such that R=Q-P on E_A.
  ...
  // Output: the coefficient A corresponding to the curve E_A: y^2=x^3+A*x^2+x.   
    ...
    fpcopy((digit_t*)&Montgomery_one, one[0]);
    fp2add(xP, xQ, t1);                           // t1 = xP+xQ
    fp2mul_mont(xP, xQ, t0);                      // t0 = xP*xQ    /* parallel 1 */
    fp2mul_mont(xR, t1, t2);                      // t2 = xR*t1    /* parallel 1 */
    fp2add(t0, t2, t2);                           // t2 = t2+t0
    fp2sub(t2, one, t2);                          // t2 = t2-1
    fp2mul_mont(t0, xR, t0);                      // t0 = t0*xR    /* parallel 2 */
    fp2sqr_mont(t2, t2);                          // t2 = t2^2     /* parallel 2 */
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2add(t1, xR, t1);                           // t1 = t1+xR
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2inv_mont_ct(t0);                           // t0 = 1/t0
    fp2mul_mont(t2, t0, t2);                      // t2 = t2*t0    /* serial */
    fp2sub(t2, t1, A);                            // A = t2-t1
#endif
}
...
{ // Computes the j-invariant of a Montgomery curve with projective constant.
  ...
  // Output: j=256*(A^2-3*C^2)^3/(C^4*(A^2-4*C^2)), which is the j-invariant of the Montgomery curve B*y^2=x^3+(A/C)*x^2+x or (equivalently) j-invariant of B'*y^2=C*x^3+A*x^2+C*x.   
    ...
    fp2sqr_mont(A, t3);                           // t3 = A^2        /* parallel 1 */  
    fp2sqr_mont(C, t1);                           // t1 = C^2        /* parallel 1 */
    fp2add(t1, t1, t0);                           // t0 = t1+t1
    fp2sub(t3, t0, t0);                           // t0 = t3-t0
    fp2sub(t0, t1, t0);                           // t0 = t0-t1
    fp2sub(t0, t1, t3);                           // t3 = t0-t1
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2sqr_mont(t1, t1);                          // t1 = t1^2       /* parallel 2 */ 
    fp2sqr_mont(t0, t2);                          // t2 = t0^2       /* parallel 2 */ 
    fp2mul_mont(t3, t1, t3);                      // t3 = t3*t1      /* parallel 3 */ 
    fp2mul_mont(t0, t2, t0);                      // t0 = t0*t2      /* parallel 3 */ 
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2inv_mont_ct(t3);                           // t3 = 1/t3 
    fp2mul_mont(t3, t0, jinv);                    // jinv = t0*t3    /* serial */ 
#endif
}
...
{ // Simultaneous doubling and differential addition.
  ...
  // Output: projective Montgomery points P <- 2*P = (X2P:Z2P) such that x(2P)=X2P/Z2P, and Q <- P+Q = (XQP:ZQP) such that = x(Q+P)=XQP/ZQP.
    ...
    mp2_add(P->X, P->Z, t0);                        // t0 = XP+ZP
    mp2_sub_p2(P->X, P->Z, t1);                     // t1 = XP-ZP
    mp2_add(Q->X, Q->Z, t2);                        // t2 = XQ+ZQ
    mp2_sub_p2(Q->X, Q->Z, t3);                     // t3 = XQ-ZQ
    fp2mul_mont(t1, t2, t2);                        // t2 = (XP-ZP)*(XQ+ZQ)   /* parallel 1 */
    fp2mul_mont(t0, t3, t3);                        // t3 = (XP+ZP)*(XQ-ZQ)   /* parallel 1 */
    mp2_sub_p2(t3, t2, t4);                         // t4 = (XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)
    mp2_add(t2, t3, t2);                            // t2 = (XP+ZP)*(XQ-ZQ)+(XP-ZP)*(XQ+ZQ)
    fp2sqr_mont(t0, t0);                            // t0 = (XP+ZP)^2    /* parallel 2 */
    fp2sqr_mont(t1, t1);                            // t1 = (XP-ZP)^2    /* parallel 2 */
    mp2_sub_p2(t0, t1, t3);                         // t3 = (XP+ZP)^2-(XP-ZP)^2
    fp2mul_mont(t0, t1, P->X);                      // XP = (XP+ZP)^2*(XP-ZP)^2         /* parallel 3 */
    fp2mul_mont(A24, t3, t0);                       // t0 = A24*[(XP+ZP)^2-(XP-ZP)^2]   /* parallel 3 */
    mp2_add(t0, t1, t1);                            // t1 = A24*[(XP+ZP)^2-(XP-ZP)^2]+(XP-ZP)^2
    fp2sqr_mont(t4, t4);                            // t4 = [(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2   /* parallel 4 */
    fp2sqr_mont(t2, Q->X);                          // XQ = [(XP+ZP)*(XQ-ZQ)+(XP-ZP)*(XQ+ZQ)]^2   /* parallel 4 */
    fp2mul_mont(t1, t3, P->Z);                      // ZP = [A24*[(XP+ZP)^2-(XP-ZP)^2]+(XP-ZP)^2]*[(XP+ZP)^2-(XP-ZP)^2]   /* parallel 5 */
    fp2mul_mont(t4, xPQ, Q->Z);                     // ZQ = xPQ*[(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2                       /* parallel 5 */
#endif
}
...
{ // Differential addition with precomputed doubling point.
  ...
  // Output: projective Montgomery point Q <- P2+Q = (XQP:ZQP) such that = x(Q+P2)=XQP/ZQP.
    ...
    mp2_sub_p2(t3, t2, t4);                         // t4 = (XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)
    mp2_add(t2, t3, t2);                            // t2 = (XP+ZP)*(XQ-ZQ)+(XP-ZP)*(XQ+ZQ)
    fp2sqr_mont(t4, t4);                            // t4 = [(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2   /* parallel 2 */
    fp2sqr_mont(t2, Q->X);                          // XQ = [(XP+ZP)*(XQ-ZQ)+(XP-ZP)*(XQ+ZQ)]^2   /* parallel 2 */
    fp2mul_mont(t4, xPQ, Q->Z);                     // ZQ = xPQ*[(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2        /* serial */
#endif
}
...
static void LADDER3PT(const f2elm_t xP, const f2elm_t xQ, const f2elm_t xPQ, const unsigned char* m, const unsigned int AliceOrBob, point_proj_t R, const f2elm_t A)
{
    ...
    for (i = 0; i < nbits; i++) {
        if (((m[i >> 3] >> (i & 7)) & 1) == 0) {
            xDBLADD(R0, R2, R->X, A24);
            fp2mul_mont(R2->X, R->Z, R2->X);    /* serial */
        } else {
            xDBLADD(R0, R, R2->X, A24);
            fp2mul_mont(R->X, R2->Z, R->X);     /* serial */
        }
    }
}
