...
#include "../src/P128/P128_internal.h"
...
{ // Tests for the field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing field arithmetic over GF(p128): \n\n"); 

    // Field addition over the prime p128
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom128_test(a); fprandom128_test(b); fprandom128_test(c); fprandom128_test(d); fprandom128_test(e); fprandom128_test(f); 

        fpadd128(a, b, d); fpadd128(d, c, e);                 // e = (a+b)+c
        fpadd128(b, c, d); fpadd128(d, a, f);                 // f = a+(b+c)
        fpcorrection128(e);
        fpcorrection128(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpadd128(a, b, d);                                     // d = a+b 
        fpadd128(b, a, e);                                     // e = b+a
        fpcorrection128(d);
        fpcorrection128(e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero128(b);
        fpadd128(a, b, d);                                     // d = a+0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero128(b);
        fpcopy128(a, d);     
        fpneg128(d);                      
        fpadd128(a, d, e);                                     // e = a+(-a)
        fpcorrection128(e);
    ...
    // Field subtraction over the prime p128
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom128_test(a); fprandom128_test(b); fprandom128_test(c); fprandom128_test(d); fprandom128_test(e); fprandom128_test(f); 

        fpsub128(a, b, d); fpsub128(d, c, e);                 // e = (a-b)-c
        fpadd128(b, c, d); fpsub128(a, d, f);                 // f = a-(b+c)
        fpcorrection128(e);
        fpcorrection128(f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }

        fpsub128(a, b, d);                                     // d = a-b 
        fpsub128(b, a, e);                                         
        fpneg128(e);                                           // e = -(b-a)
        fpcorrection128(d);
        fpcorrection128(e);
        ...
        fpzero128(b);
        fpsub128(a, b, d);                                     // d = a-0 
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero128(b);
        fpcopy128(a, d);                 
        fpsub128(a, d, e);                                     // e = a+(-a)
        fpcorrection128(e);
        if (compare_words(e, b, NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Field multiplication over the prime p128
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fprandom128_test(a); fprandom128_test(b); fprandom128_test(c);  
        fprandom128_test(ma); fprandom128_test(mb); fprandom128_test(mc); fprandom128_test(md); fprandom128_test(me); fprandom128_test(mf); 
        ...
        fpmul128_mont(ma, mb, md); fpmul128_mont(md, mc, me);                          // e = (a*b)*c
        fpmul128_mont(mb, mc, md); fpmul128_mont(md, ma, mf);                          // f = a*(b*c)
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_mont(a, ma); to_mont(b, mb); to_mont(c, mc); 
        fpadd128(mb, mc, md); fpmul128_mont(ma, md, me);                               // e = a*(b+c)
        fpmul128_mont(ma, mb, md); fpmul128_mont(ma, mc, mf); fpadd128(md, mf, mf);    // f = a*b+a*c
        from_mont(me, e);
        from_mont(mf, f);
        if (compare_words(e, f, NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_mont(a, ma); to_mont(b, mb);
        fpmul128_mont(ma, mb, md);                                                      // d = a*b 
        fpmul128_mont(mb, ma, me);                                                      // e = b*a 
        from_mont(md, d);
        from_mont(me, e);
        if (compare_words(d, e, NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_mont(a, ma);
        fpzero128(b); b[0] = 1; to_mont(b, mb);
        fpmul128_mont(ma, mb, md);                                                      // d = a*1  
        from_mont(ma, a);
        from_mont(md, d);                
        if (compare_words(a, d, NWORDS_FIELD)!=0) { passed=0; break; }
        
        fpzero128(b); to_mont(b, mb);
        fpmul128_mont(ma, mb, md);                                                      // d = a*0  
        from_mont(mb, b);
        from_mont(md, d);                
    ...
    // Field squaring over the prime p128
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom128_test(a);
        ...
        to_mont(a, ma);
        fpsqr128_mont(ma, mb);                                 // b = a^2
        fpmul128_mont(ma, ma, mc);                             // c = a*a 
        if (compare_words(mb, mc, NWORDS_FIELD)!=0) { passed=0; break; }

        fpzero128(a); to_mont(a, ma);
        fpsqr128_mont(ma, md);                                 // d = 0^2 
    ...
    // Field inversion over the prime p128
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fprandom128_test(a); 
        to_mont(a, ma);
        fpzero128(d); d[0]=1; to_mont(d, md);
        fpcopy128(ma, mb);                            
        fpinv128_mont(ma);                                
        fpmul128_mont(ma, mb, mc);                             // c = a*a^-1 
...
{ // Tests for the quadratic extension field arithmetic
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Testing quadratic extension arithmetic over GF(p128^2): \n\n"); 

    // Addition over GF(p128^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random128_test((digit_t*)a); fp2random128_test((digit_t*)b); fp2random128_test((digit_t*)c); fp2random128_test((digit_t*)d); fp2random128_test((digit_t*)e); fp2random128_test((digit_t*)f); 

        fp2add128(a, b, d); fp2add128(d, c, e);                 // e = (a+b)+c
        fp2add128(b, c, d); fp2add128(d, a, f);                 // f = a+(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2add128(a, b, d);                                     // d = a+b 
        fp2add128(b, a, e);                                     // e = b+a
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero128(b);
        fp2add128(a, b, d);                                     // d = a+0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero128(b);
        fp2copy128(a, d);     
        fp2neg128(d);                      
        fp2add128(a, d, e);                                     // e = a+(-a)
    ...
    // Subtraction over GF(p128^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random128_test((digit_t*)a); fp2random128_test((digit_t*)b); fp2random128_test((digit_t*)c); fp2random128_test((digit_t*)d); fp2random128_test((digit_t*)e); fp2random128_test((digit_t*)f); 

        fp2sub128(a, b, d); fp2sub128(d, c, e);                 // e = (a-b)-c
        fp2add128(b, c, d); fp2sub128(a, d, f);                 // f = a-(b+c)
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2sub128(a, b, d);                                     // d = a-b 
        fp2sub128(b, a, e);                                         
        fp2neg128(e);                                           // e = -(b-a)
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero128(b);
        fp2sub128(a, b, d);                                     // d = a-0 
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero128(b);
        fp2copy128(a, d);                 
        fp2sub128(a, d, e);                                     // e = a+(-a)
        if (compare_words((digit_t*)e, (digit_t*)b, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    // Multiplication over GF(p128^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {    
        fp2random128_test((digit_t*)a); fp2random128_test((digit_t*)b); fp2random128_test((digit_t*)c);  
        fp2random128_test((digit_t*)ma); fp2random128_test((digit_t*)mb); fp2random128_test((digit_t*)mc); fp2random128_test((digit_t*)md); fp2random128_test((digit_t*)me); fp2random128_test((digit_t*)mf); 
        ...
        fp2mul128_mont(ma, mb, md); fp2mul128_mont(md, mc, me);                          // e = (a*b)*c
        fp2mul128_mont(mb, mc, md); fp2mul128_mont(md, ma, mf);                          // f = a*(b*c)
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
      
        to_fp2mont(a, ma); to_fp2mont(b, mb); to_fp2mont(c, mc); 
        fp2add128(mb, mc, md); fp2mul128_mont(ma, md, me);                               // e = a*(b+c)
        fp2mul128_mont(ma, mb, md); fp2mul128_mont(ma, mc, mf); fp2add128(md, mf, mf);   // f = a*b+a*c
        from_fp2mont(me, e);
        from_fp2mont(mf, f);
        if (compare_words((digit_t*)e, (digit_t*)f, 2*NWORDS_FIELD)!=0) { passed=0; break; }
       
        to_fp2mont(a, ma); to_fp2mont(b, mb);
        fp2mul128_mont(ma, mb, md);                                                      // d = a*b 
        fp2mul128_mont(mb, ma, me);                                                      // e = b*a 
        from_fp2mont(md, d);
        from_fp2mont(me, e);
        if (compare_words((digit_t*)d, (digit_t*)e, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        to_fp2mont(a, ma);
        fp2zero128(b); b[0][0] = 1; to_fp2mont(b, mb);
        fp2mul128_mont(ma, mb, md);                                                      // d = a*1  
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
        
        fp2zero128(b); to_fp2mont(b, mb);
        fp2mul128_mont(ma, mb, md);                                                      // d = a*0 
        from_fp2mont(md, d);               
    ...
    // Squaring over GF(p128^2)
    passed = 1;
    for (n=0; n<TEST_LOOPS; n++)
    {
        fp2random128_test((digit_t*)a);
        
        to_fp2mont(a, ma);
        fp2sqr128_mont(ma, mb);                                 // b = a^2
        fp2mul128_mont(ma, ma, mc);                             // c = a*a 
        from_fp2mont(mb, b);               
        from_fp2mont(mc, c);               
        if (compare_words((digit_t*)b, (digit_t*)c, 2*NWORDS_FIELD)!=0) { passed=0; break; }

        fp2zero128(a); to_fp2mont(a, ma);
        fp2sqr128_mont(ma, md);                                 // d = 0^2 
        from_fp2mont(md, d);               
        if (compare_words((digit_t*)a, (digit_t*)d, 2*NWORDS_FIELD)!=0) { passed=0; break; }
    }
    ...
    for (n=0; n<TEST_LOOPS; n++)
    {
        ...
        to_fp2mont(a, ma);
        fp2zero128(d); d[0][0]=1; to_fp2mont(d, md);
        fp2copy128(ma, mb);                            
        fp2inv128_mont(ma);                                
        fp2mul128_mont(ma, mb, mc);                             // c = a*a^-1              
        from_fp2mont(mc, c);  
...
bool fp_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking field arithmetic over GF(p128): \n\n"); 
        
    fprandom128_test(a); fprandom128_test(b); fprandom128_test(c);

    // GF(p) addition using p128
    cycles = 0;
    ...
    // GF(p) multiplication using p128
    cycles = 0;
    for (n=0; n<BENCH_LOOPS; n++)
    {
        cycles1 = cpucycles(); 
...
bool fp2_run()
{
    ...
    printf("\n--------------------------------------------------------------------------------------------------------\n\n"); 
    printf("Benchmarking quadratic extension arithmetic over GF(p128^2): \n\n"); 
...
bool ecisog_run()
{
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random128_test((digit_t*)A24); fp2random128_test((digit_t*)C24);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random128_test((digit_t*)A); fp2random128_test((digit_t*)coeff[0]); fp2random128_test((digit_t*)coeff[1]); fp2random128_test((digit_t*)coeff[2]);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random128_test((digit_t*)A4); fp2random128_test((digit_t*)C);
    ...
    for (n=0; n<BENCH_LOOPS; n++)
    {
        fp2random128_test((digit_t*)A); fp2random128_test((digit_t*)C);
...
int main()
{
    ...
    OK = OK && fp_test();          // Test field operations using p128
    OK = OK && fp_run();           // Benchmark field operations using p128

    OK = OK && fp2_test();         // Test arithmetic functions over GF(p128^2)
    OK = OK && fp2_run();          // Benchmark arithmetic functions over GF(p128^2)
