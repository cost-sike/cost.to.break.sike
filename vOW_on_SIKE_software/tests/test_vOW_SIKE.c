...
{
    uint32_t random_functions, collisions, mem_collisions, dist_points, number_steps_collect, number_steps_locate, number_steps;
    bool success;
    ...
    if (!collect_stats) {
        ...
        printf("Expected number of distinct collisions per function (c), expected 1.1w = %.2f\n", 1.1 * pow(2, (double)MEMORY_LOG_SIZE));
        printf("\n");
        printf("Expected number of function versions (n/(2c)), expected 0.45n/w = %.2f\n",
                0.45 * pow(2, insts_constants.e - 1) / pow(2, (double)MEMORY_LOG_SIZE));
        printf("Expected total run-time (in/(2c)), expected 2.5%cn^3/w = %.2f\n", 251, 2.5 * sqrt(pow(pow(2, insts_constants.e - 1), 3) / pow(2, MEMORY_LOG_SIZE)));
    }
