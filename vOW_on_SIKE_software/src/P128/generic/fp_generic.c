/********************************************************************************************
* Portable modular arithmetic for P128
*********************************************************************************************/

#include "../P128_internal.h"

// Global constants
extern const uint64_t p128[NWORDS64_FIELD];
extern const uint64_t Montgomery_rprime1[NWORDS64_FIELD];
extern const uint64_t p128x2[NWORDS64_FIELD];
extern const uint64_t p128x4[NWORDS64_FIELD];


__inline void mp_sub128_p2(const digit_t* a, const digit_t* b, digit_t* c)
{ // Multiprecision subtraction with correction with 2*p, c = a-b+2p.
    ...
    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(borrow, c[i], ((digit_t*)p128x2)[i], borrow, c[i]); 
    }
} 


__inline void mp_sub128_p4(const digit_t* a, const digit_t* b, digit_t* c)
{ // Multiprecision subtraction with correction with 4*p, c = a-b+4p.
    ...
    borrow = 0;
    for (i = 0; i < NWORDS_FIELD; i++) {
        ADDC(borrow, c[i], ((digit_t*)p128x4)[i], borrow, c[i]); 
    }
}
...
{ // Efficient Montgomery reduction using comba and exploiting the special form of the prime p128.
  ...
  // ma is assumed to be in Montgomery representation.
    ...
    for (i = 0; i < NWORDS_FIELD; i++) {
        for (j = 0; j < i; j++) {
            MUL(mc[j], ((digit_t*)p128)[i-j], UV+1, UV[0]);
            ADDC(0, UV[0], v, carry, v); 
            ADDC(carry, UV[1], u, carry, u); 
            t += carry; 
        }
        ...
        t += carry; 
        MUL(v, ((digit_t*)Montgomery_rprime1)[0], UV+1, UV[0]);
        mc[i] = UV[0];
        MUL(mc[i], ((digit_t*)p128)[0], UV+1, UV[0]);
        ADDC(0, UV[0], v, carry, v); 
        ADDC(carry, UV[1], u, carry, u); 
        t += carry; 
        v = u;
